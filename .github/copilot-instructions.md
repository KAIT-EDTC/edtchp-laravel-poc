...
- 複数ファイルを変更していても、「同じ目的」であれば1コミットにまとめる。
- 目的が異なる変更は、同じ画面作業内でも必ず分割する。
  - 例: `docs`（資料追加）と `feat`（画面改修）は分ける。
  - 例: `home` 改修と `layout` 改修は影響範囲が異なるため分ける。
- 分割に迷った場合は「このコミットを単体で revert して意味が通るか」で判断する。

### 2.2 コミットメッセージの命名ルール補足（必須）

- メッセージは「何を実現したか」を書く。ファイル名ベースの要約は避ける。
- `app.css以外` のような「除外対象ファイル」を示す表現は禁止する。
- `app.cssを更新` のような「変更ファイル名だけ」の要約も禁止する。

NG例:

- `chore(repo): app.css以外の変更をコミット`
- `feat: app.cssを修正`

OK例:

- `feat(home): トップページに活動紹介と最新表示を追加`
- `feat(layout): ヘッダーナビにAbout導線を追加`
- `docs(about): EDTC紹介文書を追加`
